...
$(document).on("add.cards change.cards",function(b){
    ...
    if($(b.target).hasClass("mbr-instagram-feed")){
        ...
        var d=function(){c.$content.empty();c.$content.append('<div class="inst__error">Error download Instagram data!</div>');c.hideLoader()},
        a=$(b.target),
        c={$loader:$('<div class="inst__loader-wrapper">\n   <div class="inst__loader">\n      <span></span>\n      <span></span>\n      <span></span>\n   </div>\n</div>'),$content:a.find(".inst__content"),
$moreButton:a.find(".inst__more a"),showLoader:function(){$(".inst__loader-wrapper").length||a.append(this.$loader)},hideLoader:function(){this.$loader.remove()}},
        h=a.attr("data-token"),
        ...
        var k;
        if(h&&(e?k="grid":f&&(k="slider"),k))switch(b?a.find(".container_toggle").addClass("container-fluid").removeClass("container"):a.find(".container_toggle").addClass("container").removeClass("container-fluid"),k){
            case "grid":c.showLoader();
            m("users/self/media/recent",{access_token:h,count:e*p})
            .then(function(a,b){
                function f(){
                    ...
                    a.css({height:c.$content.find(".inst-card").outerWidth()})
                }
                function q(a,b){
                    for(var d=0,e=a.length;d<e;d++)c.$content.find(".inst__images").append(n(a[d],g));$(window).on("resize",function(c){f()});f();b?c.$moreButton.attr("data-max-id",b):c.$moreButton.remove()
                }
                c.hideLoader();
                c.$content.empty();
                ...
                c.$moreButton.on("click",function(a){
                    ...
                    window.mbrAppCore||(
                        a.preventDefault(),
                        c.showLoader(),
                        m("users/self/media/recent",{access_token:h,count:e*p,max_id:b})
                        .done(function(a,b){q(a,b);
                            c.hideLoader()})
                        .fail(d))
                    })
                })
            .fail(d);
            break;
            case "slider":c.showLoader(),
            m("users/self/media/recent",{access_token:h,count:3*f})
            .then(function(a,b){
                var d=b?b:null;
                c.hideLoader();
                c.$content.empty();
                ...
                l.on("init, setPosition",function(a){
                    ...
                    l.on("afterChange",function(a,b,e){
                        e+ +f>=b.slideCount&&d&&(c.showLoader(),
                        m("users/self/media/recent",{access_token:h,count:2*f,max_id:d})
                        .then(function(a,b){
                            var e=[];c.hideLoader();
                            for(var f=0,h=a.length;f<h;f++)e.push(n(a[f],g));l.slick("slickAdd",e.join("\n"));l.find(".slick-arrow").remove();l.slick("reinit");d=b?b:null}))})}).fail(d)}
...
//En esta linea hicimos el cambio:
